...
use syn::{
    parse_macro_input, parse_str, spanned::Spanned, AttrStyle, Attribute, Data, GenericArgument,
    Ident, LitStr, Meta, PathArguments, Type,
};
...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let build_method_iter = struct_fields.iter().map(|field| {
        let expanded_name = Ident::new(field.name.as_str(), Span::call_site());
    ...
    let fields_with_attrs = struct_fields
        ...
        .filter(|field| {
            ...
            // field
            field.attr.is_some() && field.attr.as_ref().unwrap() != &field.name
        })
        .collect::<Vec<&StructField>>();

    let each_builder_method = struct_fields.iter().map(|field| {
        let expanded_name = Ident::new(field.name.as_str(), Span::call_site());
        let expanded_type = &field.ty;
    ...
    let each_builder_att_method = fields_with_attrs.iter().map(|field| {
        ...
        let field_attr_ident = Ident::new(field.attr.as_ref().unwrap(), Span::call_site());
        quote! {
    ...
    let builder_struct_atts = struct_fields.iter().map(|field| {
        if field.attr.is_some() && field.attr.as_ref().unwrap() != &field.name {
            // For attributes, we assume that its type is always a `Vec<String>`
            let expanded_name = Ident::new(field.attr.as_ref().unwrap(), Span::call_site());
            quote! {
